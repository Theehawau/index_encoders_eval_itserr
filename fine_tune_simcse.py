...
class SimCSETrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None):
        ...
        greek_syn_q_embs = get_embeddings("training_data/eval_data/100_query_greek.txt", model,  self.tokenizer)
        greek_syn_t_embs = get_embeddings("training_data/eval_data/500_target_greek.txt", model, self.tokenizer)
        latin_syn_q_embs = get_embeddings("training_data/eval_data/100_query_latin.txt", model,  self.tokenizer)
        latin_syn_t_embs = get_embeddings("training_data/eval_data/500_target_latin.txt", model, self.tokenizer)
        greek_real_q_embs = get_embeddings("training_data/eval_data/greco_latino_veri/20_query_greek.txt", model, self.tokenizer)
        latin_real_q_embs = get_embeddings("training_data/eval_data/greco_latino_veri/20_query_latin.txt", model, self.tokenizer)
        greek_real_t_embs = get_embeddings("training_data/eval_data/greco_latino_veri/100_target_greek.txt", model, self.tokenizer)
        latin_real_t_embs = get_embeddings("training_data/eval_data/greco_latino_veri/100_target_latin.txt", model, self.tokenizer)
...
def main(args):
    ...
    dataset = load_dataset("csv", data_files={"train": args.data_path})["train"]
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fine-tune SimCSE model.")
    parser.add_argument("--data_path", type=str, required=True)
    # Aggiungi qui eventuali argomenti da passare
