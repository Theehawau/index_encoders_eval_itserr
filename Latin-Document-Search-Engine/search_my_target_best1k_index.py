...
def main(argv):
    ...
    out_dir = f"target_random1k_data/{FLAGS.index_model}_index/_outputs_{H.model.model.replace('/', '_')}"
    ...
    df = pd.read_csv("data/Latin_benchmark.txt", sep="\t")
    ...
    if FLAGS.get_targets:
        print(f"Saving targets to: target_random1k_data/{FLAGS.index_model}_index/Latin_benchmark_best_targets.txt")
        out_file = open(f"target_random1k_data/{FLAGS.index_model}_index/Latin_benchmark_best_targets.txt", "w", encoding="utf-8")
    ...
    df = pd.read_csv(f"target_random1k_data/{FLAGS.index_model}_index/Latin_benchmark_best_targets.txt", sep="\t")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, 
                        default="/home/hawau/Latin-Document-Search-Engine/config/target_index_config.py", 
