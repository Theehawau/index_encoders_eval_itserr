...
def main(argv):
    ...
    out_dir = f"{H.data.save_index_output_dir}/_outputs_{H.model.model.replace('/', '_')}"
    ...
    df = pd.read_csv(f"{H.data.save_index_output_dir}/Greek_benchmark.txt", sep="\t")
    ...
    if FLAGS.get_targets:
        out_file = open(f"{H.data.save_index_output_dir}/Greek_benchmark_best_targets.txt", "w", encoding="utf-8")
        ...
        for i, row in tqdm(df.iterrows(), total=len(df), desc="Processing Queries", unit="query"):
            ...
            for i,idx in enumerate(ann):
                ...
                outs.append(f"{rows[0][4]}")

            print('\t'.join(outs), file=out_file)

        out_file.close()

    df = pd.read_csv(f"{H.data.save_index_output_dir}/Greek_benchmark_best_targets.txt", sep="\t")
