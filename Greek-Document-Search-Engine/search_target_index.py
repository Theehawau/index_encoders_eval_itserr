...
def main(argv):
    ...
    out_dir = f"target_data/{FLAGS.index_model}_index/_outputs_{H.model.model.replace('/', '_')}"
    ...
    df = pd.read_csv("data/Greek_benchmark.txt", sep="\t")
    ...
    if FLAGS.get_targets:
        out_file = open(f"target_data/{FLAGS.index_model}_index/Greek_benchmark_best_targets.txt", "w", encoding="utf-8")
    ...
    df = pd.read_csv(f"target_data/{FLAGS.index_model}_index/Greek_benchmark_best_targets.txt", sep="\t")
